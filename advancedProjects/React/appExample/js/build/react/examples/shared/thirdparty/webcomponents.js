...
if (WebComponents.flags.shadow) {
  ...
  (function (scope) {
    ...
    mixin(HTMLSelectElement.prototype, {
      add: function add(element, before) {
        if ((typeof before === 'undefined' ? 'undefined' : _typeof(before)) === "object") before = unwrap(before);
      ...
      remove: function remove(indexOrNode) {
        ...
        if ((typeof indexOrNode === 'undefined' ? 'undefined' : _typeof(indexOrNode)) === "object") indexOrNode = unwrap(indexOrNode);
  ...
  (function (scope) {
    ...
    Selection.prototype = {
      ...
      extend: function extend(node, offset) {
  ...
  (function (scope) {
    var ShadowCSS = {
      strictStyling: false,
      registry: {},
      shimStyling: function shimStyling(root, name, extendsName) {
      ...
      makeScopeSelector: function makeScopeSelector(name, typeExtension) {
        if (name) {
          return typeExtension ? "[is=" + name + "]" : name;
        }
        return "";
      },
      isTypeExtension: function isTypeExtension(extendsName) {
        return extendsName && extendsName.indexOf("-") < 0;
      },
      prepareRoot: function prepareRoot(root, name, extendsName) {
      ...
      removeStyles: function removeStyles(root, styles) {
        for (var i = 0, l = styles.length, s; i < l && (s = styles[i]); i++) {
          s.parentNode.removeChild(s);
        }
      },
      registerRoot: function registerRoot(root, name, extendsName) {
      ...
      findStyles: function findStyles(root) {
        if (!root) {
          return [];
        }
      ...
      applyScopeToContent: function applyScopeToContent(root, name) {
        if (root) {
      ...
      insertDirectives: function insertDirectives(cssText) {
        cssText = this.insertPolyfillDirectivesInCssText(cssText);
        return this.insertPolyfillRulesInCssText(cssText);
      },
      insertPolyfillDirectivesInCssText: function insertPolyfillDirectivesInCssText(cssText) {
      ...
      insertPolyfillRulesInCssText: function insertPolyfillRulesInCssText(cssText) {
      ...
      scopeCssText: function scopeCssText(cssText, scopeSelector) {
      ...
      extractUnscopedRulesFromCssText: function extractUnscopedRulesFromCssText(cssText) {
        var r = "",
            m;
      ...
      convertColonHost: function convertColonHost(cssText) {
        return this.convertColonRule(cssText, cssColonHostRe, this.colonHostPartReplacer);
      },
      convertColonHostContext: function convertColonHostContext(cssText) {
        return this.convertColonRule(cssText, cssColonHostContextRe, this.colonHostContextPartReplacer);
      },
      convertColonRule: function convertColonRule(cssText, regExp, partReplacer) {
      ...
      colonHostContextPartReplacer: function colonHostContextPartReplacer(host, part, suffix) {
      ...
      colonHostPartReplacer: function colonHostPartReplacer(host, part, suffix) {
        return host + part.replace(polyfillHost, "") + suffix;
      },
      convertShadowDOMSelectors: function convertShadowDOMSelectors(cssText) {
        for (var i = 0; i < shadowDOMSelectorsRe.length; i++) {
          cssText = cssText.replace(shadowDOMSelectorsRe[i], " ");
        }
        return cssText;
      },
      scopeRules: function scopeRules(cssRules, scopeSelector) {
        var cssText = "";
        if (cssRules) {
      ...
      scopeSelector: function scopeSelector(selector, _scopeSelector, strict) {
        var r = [],
      ...
      insertPolyfillHostInCssText: function insertPolyfillHostInCssText(selector) {
      ...
      replaceTextInStyles: function replaceTextInStyles(styles, action) {
        if (styles && action) {
      ...
      addCssToDocument: function addCssToDocument(cssText, name) {
        if (cssText.match("@import")) {
          addOwnSheet(cssText, name);
        } else {
          _addCssToDocument(cssText);
        }
      }
    };
    ...
    function _addCssToDocument(cssText) {
      if (cssText) {
    ...
    if (window.ShadowDOMPolyfill) {
      _addCssToDocument("style { display: none !important; }\n");
...
(function (scope) {
  ...
  var polymerStub = function polymerStub(name, dictionary) {
